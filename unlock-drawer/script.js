...
// Square icon proportions — easy to change later if you want a different shape.
...
function saveToLocalStorage() {
  try {
    localStorage.setItem('unlockDrawerStrokes', JSON.stringify(strokes));
  } catch (e) {
...
function loadFromLocalStorage() {
  try {
    const raw = localStorage.getItem('unlockDrawerStrokes')
      || localStorage.getItem('lockDrawerStrokes');
    if (raw) {
...
saveBtn.addEventListener('click', () => {
  ...
  downloadFile('unlock-drawing-strokes.json', JSON.stringify(strokes), 'application/json');
});
