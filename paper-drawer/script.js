...
// Portrait paper proportions, sized to sit within the clipboard's board area.
...
// The paper itself is white — that's baked into the saved PNG on purpose.
function drawPaperFill() {
  cardOutlinePath();
  ctx.save();
...
function saveToLocalStorage() {
  try {
    localStorage.setItem('paperDrawerStrokes', JSON.stringify(strokes));
  } catch (e) {
...
function loadFromLocalStorage() {
  try {
    const raw = localStorage.getItem('paperDrawerStrokes');
    if (raw) {
...
saveBtn.addEventListener('click', () => {
  ...
  downloadFile('paper-drawing-strokes.json', JSON.stringify(strokes), 'application/json');
});
